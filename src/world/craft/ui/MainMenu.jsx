    ...
    >
      ...
      <div className="relative flex w-full max-w-[520px] flex-1 flex-col items-center justify-center px-4 pb-24 pt-[7svh] max-sm:pb-20">
        ...
        <div className="mt-5 flex flex-col items-center gap-1.5 text-center">
          <p className="rv text-sm uppercase tracking-[0.35em] text-[#f4f1ea]/95 [text-shadow:1px_1px_0_rgba(0,0,0,0.8)] sm:text-base" style={{ animationDelay: '260ms' }}>
            Naman Singh
          </p>
          <p className="rv text-xs uppercase tracking-[0.28em] text-[#c9d4cb]/85 [text-shadow:1px_1px_0_rgba(0,0,0,0.8)]" style={{ animationDelay: '380ms' }}>
        ...
        <nav className="mt-10 flex w-full flex-col items-stretch gap-3" aria-label="Main menu">
          <div className="rv" style={{ animationDelay: '600ms' }}>
            <button
              type="button"
          ...
          <div className="grid grid-cols-2 gap-3 max-sm:grid-cols-1">
            {SECTIONS.map((s, i) => (
              <div
                key={s.key}
                className={`rv ${i === SECTIONS.length - 1 ? 'col-span-2 max-sm:col-span-1' : ''}`}
                style={{ animationDelay: `${690 + i * 70}ms` }}
              >
                <button
                  type="button"
                  disabled={!ready}
                  ...
                  onMouseEnter={sfx.hover}
                  className={`mc-btn font-pixel flex h-[46px] w-full items-center justify-center gap-2.5 text-[11px]`}
                >
